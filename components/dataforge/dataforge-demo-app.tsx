...
const idleQualityCopy: Record<ReportMode, string[]> = {
  baseline: ["Drop a ZIP or click \"Load demo animal dataset\" to begin."],
  measured: ["Awaiting a dataset. The GPT evaluator has not been called."],
  inferred: ["The repair plan will be generated by GPT-5.5 once the dataset is analyzed."],
...
const EMPTY_QUALITY_REPORT: DataForgeQualityReport = {
  ...
  summary: "No dataset loaded. Drop a ZIP or click \"Load demo animal dataset\".",
...
export function DataForgeDemoApp() {
  ...
  }, [uploadedDataset, convexDistribution, isConvexMode]);
  ...
  const currentAugmentedDistribution = useMemo(() => {
    ...
    return buildAugmentedDistribution(convexDistribution, currentGapJobs);
  }, [uploadedDataset, liveFinalMetrics, convexDistribution, currentGapJobs]);
  ...
  // One sample image per class for the source-dataset rig preview. Uses
  // the first sample with an imageUrl whose effective label matches each
  // class. Only populated for uploads (demo data has no real image bytes).
  ...
  async function runAnalysisForUpload() {
    ...
    await step(
      ...
      "running",
      "labelize.started",
      "Calling GPT-5.5 for structured repair plan…",
    );
    ...
    await step(
      ...
      "complete",
      "labelize.complete",
      `GPT-5.5 repair plan received with ${qualityReportResult.gapJobs.length} gap job(s). Approve label and duplicate decisions in the integration band.`,
    );
  ...
  /**
   * Real GPT-5.5 quality-report call. Throws on non-2xx — there is no
  ...
  async function ingestFalGenerationResponse(
    ...
    const startCount = uploadedDataset.samples.filter(
    ...
    setUploadedDataset((current) => {
      ...
      // Recompute distribution to include the new synthetic samples.
      const classDistribution = { ...current.classDistribution };
      for (const sample of newSamples) {
        const label =
          sample.finalLabel ?? sample.currentLabel ?? sample.originalLabel;
        if (label) {
          ...
          <section className="hero-grid" aria-labelledby="heroTitle">
            ...
            >
              ...
              {uploadedDataset ? (
                ...
                // Idle: nothing loaded yet. Render an honest placeholder
                // box so the rig doesn't collapse — but no class names.
                <div
                  className="pixel-field"
                  style={{
                    display: "grid",
                  ...
                  aria-hidden="true"
                >
                  Drop a ZIP or click <code style={{ marginInline: "0.25rem" }}>Load demo animal dataset</code> to populate.
                </div>
              )}
          ...
          >
            ...
            <div className="integration-grid" style={{ display: "grid", gap: "1.25rem", gridTemplateColumns: "minmax(0, 1fr)" }}>
              ...
              {uploadedDataset && (() => {
                ...
                // Fal has generated. Cap at 5 thumbnails for display
                // purposes; the full set lives in DatasetExplorer below
                // and in the export ZIP.
                const synthetic = cleanSamples.filter((s) => s.source === "synthetic");
                if (synthetic.length === 0) return null;
                const displayed = synthetic.slice(0, 5);
                return (
                  <section
                  ...
                  >
                    <header style={{ marginBottom: "0.85rem" }}>
                      ...
                      >
                        Showing {displayed.length} of {synthetic.length} synthetic
                        sample{synthetic.length === 1 ? "" : "s"} generated by{" "}
                        <code style={{ fontSize: "0.74rem" }}>fal-ai/flux/schnell</code>{" "}
                        from GPT-authored gap-job prompts. Full set is in the
                        dataset explorer below and in the export ZIP under{" "}
