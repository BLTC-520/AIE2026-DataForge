...
if grep -q '@cloudflare/vite-plugin\|wrangler' package.json; then
  ...
  echo "   Recommendation: Use framer-motion (legacy alias) v12.23.24 instead"
  echo ""
  read -p "Continue with Motion (may cause build errors) or use framer-motion (legacy alias)? [motion/framer]: " choice
...
if [ "$USING_CLOUDFLARE" = true ]; then
  ...
  echo "   Consider using framer-motion (legacy alias) v12.23.24 if build errors occur"
  echo ""
fi
