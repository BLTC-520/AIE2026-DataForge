...
**Current Version**: 12.23.24 (verified 2026-04-23)
...
## Motion v12 Migration & Behavior Changes

Motion v12 (and v11) introduced several internal changes that affect existing codebases and tests:

### 1. Package Renaming
- **Uninstall**: `framer-motion`
- **Install**: `motion`
- **Import**: `import { motion } from "motion/react"` (or `motion/react-client` for Next.js)

### 2. Velocity Calculation
Subsequent updates within the same synchronous block of code no longer contribute to velocity. Velocity is now calculated relative to the value at the end of the previous frame.

```tsx
const x = motionValue(0)
requestAnimationFrame(() => {
  x.set(100)
  x.getVelocity() // Velocity 0 -> 100
  x.set(200)
  x.getVelocity() // Velocity 0 -> 200 (100 is ignored as instantaneous)
})
```

### 3. Render Scheduling (Testing Impact)
Motion now triggers renders on mount as a **microtask** rather than synchronously. This improves performance but requires updating Jest tests to await an animation frame.
...
// In your test:
render(<motion.div animate={{ opacity: 1 }} transition={{ duration: 0 }} />)
await nextFrame()
...
## Package Versions (Verified 2026-04-23)
