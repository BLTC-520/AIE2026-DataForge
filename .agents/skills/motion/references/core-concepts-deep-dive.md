...
```tsx
{
  ...
  velocity: { x: number, y: number }     // Current velocity (calculated relative to previous frame)
}
```

> [!NOTE]
> **Velocity in v12**: Subsequent updates within the same synchronous block are ignored for velocity calculations. Only the final state at the end of the frame (or the transition from the previous frame) counts.
